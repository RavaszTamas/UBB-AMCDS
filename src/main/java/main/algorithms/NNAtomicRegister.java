...
public class NNAtomicRegister extends AbstractMessagingAlgorithm {
...
//    System.out.println("Creating new register: " + id);
    this.queueProcessor = queueProcessor;
    var processes = this.queueProcessor.getSystems().get(pipelineExecutor.getSystemId());
    this.register =
        new Register(
            this.queueProcessor.getOwnerProcess().getRank(),
            new NNARStruct(),
            0,
            0,
            0,
...
//      System.out.println(register.getAcks());
      if (register.getAcks() > register.getReadList().length / 2) {
        ...
        this.pipelineExecutor.submitToPipeline("app.nnar[" + registerId + "].pl", newPayload);
      }
    }
  }
  ...
  private void bebBroadcastInternalRead(String systemId, String registerId, int readId) {
    CommunicationProtocol.Message message =
        MessageComposer.createBebBroadcast(
            ...
            MessageComposer.createNNARInternalRead(
                systemId, "app.nnar[" + registerId + "]", readId));
    this.pipelineExecutor.submitToPipeline("app.nnar[" + registerId + "].beb.pl", message);
  }
